...
def verify_part(md_path, pdf_path):
    ...
    try:
        with open(md_path, 'r', encoding='utf-8') as f:
         ...
         return {
            'md_path': md_path, 'pdf_path': pdf_path, 'md_lines': 0, 'md_numeral_count': 0,
            'pdf_numeral_count': 0, 'discrepancies': 0, 'broken_thai': 0, 'is_empty': True,
            'top_discrepancies': []
        }
...
def main():
    ...
    pdf_files = sorted(glob.glob(os.path.join(parts_dir, "Academic_291121_112321_part_*.pdf")))
    
    print("="*80)
    print("VOLUME 11 PART-BY-PART VERIFICATION")
    print("="*80)
    ...
    for name in common:
        ...
        diff = abs(result['md_numeral_count'] - result['pdf_numeral_count'])
        percent_diff = (diff / result['pdf_numeral_count'] * 100) if result['pdf_numeral_count'] > 0 else 0
        
        if diff > 50 or percent_diff > 10: 
             status.append(f"NUM DIFF({result['md_numeral_count']}/{result['pdf_numeral_count']})")
        
        part_num = name.split('_part_')[-1]
        
        if status:
            print(f"📄 Part {part_num:<3}: ⚠️  {', '.join(status)}")
            issues.append(result)
        else:
            print(f"📄 Part {part_num:<3}: ✅ OK (Nums: {result['md_numeral_count']}/{result['pdf_numeral_count']})")
    
    # Summary
    print("\n" + "="*80)
    print("VERIFICATION SUMMARY (Volume 11)")
    print("="*80)
    ...
    with open("vol11_verification_report.txt", "w", encoding="utf-8") as f:
        f.write("VOLUME 11 VERIFICATION REPORT\n")
        f.write("="*60 + "\n\n")
        for res in results:
