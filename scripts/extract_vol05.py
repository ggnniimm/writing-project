    ...
    for i in range(len(api_keys)):
        ...
        try:
            ...
            if "503" in str(e) or "504" in str(e) or "429" in str(e) or "ResourceExhausted" in str(e):
                safe_print(f"   ⏳ [Worker {worker_id}] API Issue ({e}). Switching key...")
                time.sleep(10)
                continue
            else:
                time.sleep(10)
...
def main():
    ...
    print(f"Found {len(pdf_files)} parts to process.")

    for i, pdf_path in enumerate(pdf_files):
        filename = os.path.basename(pdf_path)
        # Extract part number from filename (assuming part_XX.pdf)
        try:
